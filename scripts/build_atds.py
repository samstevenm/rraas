...
.tabs button { font:inherit; font-weight:bold; padding:0.4rem 0.9rem;
  cursor:pointer; border:2px solid var(--box); background:none; color:var(--ink); }
.tabs button.on { background:var(--box); color:var(--hdr-ink); }
...
def render_rickroll_page(profiles: list[tuple[str, dict]], apex: str) -> str:
    ...
    body = (
        ...
        '<div class="tabs">'
        '<button id="tab-ds" class="on" type="button">Diligent Services</button>'
        '<button id="tab-rr" type="button">RACK &amp; ROLL &#39;26</button></div>\n'
        '<div id="pane-ds">'
        ...
        '<p class="claim" id="lb-updated"></p></div>\n'
        '<div id="pane-rr" hidden>'
        '<p>A federated scoreboard from an invitational we decline to explain. '
        '<a href="https://rickroll.win/leaderboard.html">No comment</a>.</p>\n'
        '<table class="lb"><thead><tr><th>Player</th><th>Vouched by</th>'
        '<th class="n">Rolls</th></tr></thead><tbody id="rr-body">'
        '<tr><td colspan="3">Counting&hellip;</td></tr></tbody></table>\n'
        '<p class="claim" id="rr-extra"></p></div>\n'
        '</div></div>\n'
        '<script>\n'
        ...
        '  });\n'
        '// RACK & ROLL federation: cross-origin read-only JSON. The names in\n'
        '// it are attacker-adjacent, so textContent ONLY — never innerHTML.\n'
        'var rrLoaded = false;\n'
        'function showTab(rr) {\n'
        '  document.getElementById("pane-ds").hidden = rr;\n'
        '  document.getElementById("pane-rr").hidden = !rr;\n'
        '  document.getElementById("tab-ds").classList.toggle("on", !rr);\n'
        '  document.getElementById("tab-rr").classList.toggle("on", rr);\n'
        '  if (rr && !rrLoaded) { rrLoaded = true; loadRR(); }\n'
        '}\n'
        'document.getElementById("tab-ds").addEventListener("click", function () { showTab(false); });\n'
        'document.getElementById("tab-rr").addEventListener("click", function () { showTab(true); });\n'
        'function loadRR() {\n'
        '  fetch("https://rickroll.win/leaderboard.json", {cache: "no-store"})\n'
        '    .then(function (r) { return r.json(); })\n'
        '    .then(function (d) {\n'
        '      var tb = document.getElementById("rr-body");\n'
        '      tb.textContent = "";\n'
        '      (d.players || []).forEach(function (p) {\n'
        '        var tr = document.createElement("tr");\n'
        '        [String(p.display) + (p.busted ? " (definitely not curl)" : ""),\n'
        '         String(p.inviter), String(p.rolls)].forEach(function (c, i) {\n'
        ...
        '        var td = document.createElement("td");\n'
        '        td.colSpan = 3;\n'
        '        td.textContent = "awaiting the first roll.";\n'
        '        tr.appendChild(td); tb.appendChild(tr);\n'
        '      }\n'
        '      document.getElementById("rr-extra").textContent =\n'
        '        "unclaimed invitations: " + d.unclaimed_invitations + " - cowards.";\n'
        '    })\n'
        '    .catch(function () {\n'
        '      document.getElementById("rr-body").textContent = "the invitational is not answering.";\n'
        '    });\n'
        '}\n'
        '</script>\n'
